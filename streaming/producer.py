...
def cargar_test(path, subset, ds_code_map):
    ...
    for k in range(len(A)):
        eng = f"{subset}_u{int(A[k, iu])}"
        msg = {'engine_id': eng, 'cycle': int(A[k, ic]), 'Fc': int(A[k, ifc]),
               'ds_code': ds_code, 'rul_real': float(Y[k])}
        for j, s in enumerate(Xs_var): msg[s] = float(X_s[k, j])
        for j, w in enumerate(W_var):  msg[w] = float(W[k, j])
...
def main():
    ...
    data = cargar_test(h5_path, args.subset, cfg['DS_CODE'])
